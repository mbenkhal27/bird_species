...
learn.load("/Users/mbenkhal/Documents/models/resnet50-stage2")

@app.route("/")
def form(request):
    return HTMLResponse("""
        <h3>This app will classify birds <h3>
...
def predict_image_from_bytes(bytes):
    ...
    print(preds)
    prediction = preds[0]
    probablity =np.array(preds[2])[np.array(preds[1])]
    print(prediction)
    print(probablity)
    #return ({
        #"prediction":  json.dumps(str(prediction)),
        #"probablity": json.dumps(float(probablity))
    #})
    return HTMLResponse("""
    <h3> """ + json.dumps(str(prediction)) + """ </h3>
    <h3> 
    """)
